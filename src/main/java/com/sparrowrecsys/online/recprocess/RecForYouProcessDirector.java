 ...
 */
public class RecForYouProcessDirector {
    ...
    public static void main(String[] args) {
        ...
        List<Director> recList = getRecList(1, 10, "nerualcf");
        if (recList.size() == 0) {
            System.out.println("No recommendation result.");
            return;
        }
        for (Director d : recList) {
            System.out.println(d.getDirectorId() + " " + d.getName());
        }
    }
     ...
     */
    public static List<Director> getRecList(int userId, int size, String model) {
        User user = DataManager.getInstance().getUserById(userId);
        if (null == user) {
            return new ArrayList<>();
        }
        final int CANDIDATE_SIZE = 800;
        List<Director> candidates = DataManager.getInstance().getDirectors(CANDIDATE_SIZE, "popularity");
        ...
        List<Director> rankedList = ranker(user, candidates, model);
     ...
     * @param candidates director candidates
     * @param model      model name used for ranking
     * @return ranked director list
     */
    public static List<Director> ranker(User user, List<Director> candidates, String model) {
        ...
        List<Director> rankedList = new ArrayList<>();
     ...
     */
    public static double calculateEmbSimilarScore(User user, Director candidate) {
     ...
     */
    public static void callNeuralCFTFServing(User user, List<Director> candidates,
            HashMap<Director, Double> candidateScoreMap) {
        if (null == user || null == candidates || candidates.size() == 0) {
            return;
        }

        JSONArray instances = new JSONArray();
        for (Director d : candidates) {
            JSONObject instance = new JSONObject();
            instance.put("userId", user.getUserId());
            instance.put("directorId", d.getDirectorId());
        ...
        String predictionScores = asyncSinglePostRequest("http://localhost:8501/v1/models/neuralcfdirector:predict",
