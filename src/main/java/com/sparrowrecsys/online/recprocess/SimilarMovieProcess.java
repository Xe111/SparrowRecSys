...
public class SimilarMovieProcess {
    ...
    public static void main(String[] args) {
        ...
        try {
            DataManager.getInstance().loadData(basePath + "/sampledata/movies.csv",
                    ...
                    basePath + "/modeldata/item2vecEmb.csv",
                    basePath + "/modeldata/userEmb.csv",
                    "i2vEmb", "uEmb");
        } catch (Exception e) {
            e.printStackTrace();
        ...
        List<Movie> movieList = DataManager.getInstance().getMovies(1000, "rating");
        System.out.println("movieList.size() = " + movieList.size());

        List<Movie> recList = getRecList(1, 10, "emb");
        if (recList.size() == 0) {
            System.out.println("No recommendation found");
        }
        System.out.println("Recommendation list for movie "+DataManager.getInstance().getMovieById(1).getTitle()+":");
    ...
    public static double calculateSimilarScore(Movie movie, Movie candidate) {
        ...
        double genreSimilarity = (double) sameGenreCount / (movie.getGenres().size() + candidate.getGenres().size())
                / 2;
